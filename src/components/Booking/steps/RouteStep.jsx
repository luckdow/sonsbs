            ...
            <div className="absolute z-50 w-full bg-white border border-gray-200 rounded-lg shadow-lg mt-1 max-h-64 overflow-hidden">
              <div className="p-3 border-b border-gray-100 bg-blue-50">
                <p className="text-sm font-medium text-blue-700 flex items-center">
                  <span className="w-5 h-5 bg-blue-500 rounded text-white text-xs flex items-center justify-center mr-2">G</span>
                  Google Önerileri
                </p>
              </div>
              <div className="max-h-56 overflow-y-auto">
                {googlePickupSuggestions.map((place, index) => (
                  <button
                    key={`google-${index}`}
                    type="button"
                    onClick={() => handlePlaceSelect(place, true)}
                    className="w-full p-3 sm:p-4 hover:bg-blue-50 flex items-center text-left border-b border-gray-50 last:border-b-0 transition-colors"
                  >
                    <div className="flex items-center space-x-3 w-full min-w-0">
                      ...
                      <div className="flex-1 min-w-0 pr-2">
                        <div className="font-medium text-gray-900 truncate text-sm sm:text-base">{place.name}</div>
                        <div className="text-xs sm:text-sm text-gray-500 truncate">{place.address}</div>
                      </div>
                      {place.rating > 0 && (
                        <div className="flex items-center space-x-1 flex-shrink-0">
                          <Star className="w-3 h-3 sm:w-4 sm:h-4 text-yellow-400 fill-current" />
                          <span className="text-xs sm:text-sm text-gray-600">{place.rating}</span>
                        </div>
                      )}
                    </div>
                  </button>
        ...
        <div className="space-y-2">
          ...
          {showDropoffSuggestions && googleDropoffSuggestions.length > 0 && (
            <div className="absolute z-50 w-full bg-white border border-gray-200 rounded-lg shadow-lg mt-1 max-h-64 overflow-hidden">
              ...
              <div className="max-h-56 overflow-y-auto">
                {googleDropoffSuggestions.map((place, index) => (
                  <button
                    key={`google-${index}`}
                    type="button"
